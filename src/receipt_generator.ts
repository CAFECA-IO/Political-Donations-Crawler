...
const getRecords = async (
  ...
  const whereCondition: any = {}; // 動態構建 where 條件

  if (minId !== undefined) {
    whereCondition.id = { ...whereCondition.id, gte: minId }; // 添加大於等於 minId 的條件
  }

  if (maxId !== undefined) {
    whereCondition.id = { ...whereCondition.id, lte: maxId }; // 添加小於等於 maxId 的條件
  }

  if (type) {
    whereCondition.income_expenditure = { in: [type] }; // 只查詢 income 或 expenditure 的記錄
  }

  const records = await prisma.income_expenditure.findMany({
    where: Object.keys(whereCondition).length ? whereCondition : undefined, // 如果有條件則使用，否則不過濾
    take: limit, // 限制返回的記錄數量
    orderBy: {
      id: "asc", // 按照 ID 升序排序
    },
  });
...
const generateAndSaveReceipt = async (record: any) => {
  ...
  const fontBytes = fs.readFileSync(
    path.join(__dirname, "assets/fonts/SourceHanSansTW-Regular.otf")
  );
  ...
  const drawMergedTitle = (title: string, y: number) => {
    drawCellBorder(tableStartX, y, tableWidth, rowHeight); // 繪製整行的邊框
    page.drawText(title, {
      x: tableStartX + tableWidth / 2 - 100, // 調整標題位置，使其居中
      y: y + 8,
      size: 12,
      font: customFont,
      color: rgb(0, 0, 0),
  ...
  if (record.income_expenditure === "income") {
    ...
    fields = [
      ...
      // { label: "應揭露之支出對象", value: record.disclosed_recipient || "否" },
...
const batchProcessReceipts = async (batchSize = 1000) => {
  ...
  let lastProcessedId = lastSyncRecord?.endId || 0; // 從已同步的最大 endId 開始處理


  // 找到資料庫中最新的 income_expenditure id
